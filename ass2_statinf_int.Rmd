...
The t statistic is incredibly high, with a p-value near to 0. Of course is not by chance that the mean of differences between the lengths derived by dose 2 and 0.5 is higher than 0, so **the null hypothesis is rejected**.
...
Alos for dose 0.5 t statistic is higher than critical value, so **the null hypothesis is rejected**.

##Conclusions

Our t test reveals basically two things:

1. **bigger dose, bigger average difference of lengths**, but: in the context of the single delivery method, ascorbic acid enables a bigger effectiveness of dose: you see this in the higher values of the t statistics in the tests and also in the conditional boxplot of $plot3$, where there is bigger distance between the means of lengths derived from the 3 doses.
